...
class OrderDetail extends React.Component {
    ...
    //渲染数据
    componentWillMount() {
        this.props.getOrderDetail('http://192.168.21.22/cloud/pay?method=getPay', 'post', {
    ...
    ordersListHandle() {
        let self = this;
        this.props.cancelOrder('http://192.168.21.22/cloud/pay?method=payDel', 'post', {
            order_id: this.props.match.params.id.slice(1),
    ...
    payHandle() {
        ...
        switch (pay_type) {
            case 0:
                url = 'http://192.168.21.22/cloud/alipay?method=orderSign'
                break;
            case 1:
                url = ''
                alert('微信支付')
